...
class ProducerConfig:
    ...
    # Reliability
    acks: str = field(
        default_factory=lambda: os.getenv("KAFKA_PRODUCER_ACKS", "all")
    )
    retries: int = field(
...
# Singleton instance
_config: Optional[StreamPipelineConfig] = None
...
def get_config() -> StreamPipelineConfig:
    ...
    global _config
    if _config is None:
        _config = StreamPipelineConfig()
        _warn_default_secrets(_config)
    return _config


def reset_config(override: Optional[StreamPipelineConfig] = None) -> StreamPipelineConfig:
    """설정 초기화 (테스트용). override를 전달하면 해당 설정으로 대체."""
    global _config
    _config = override or StreamPipelineConfig()
    return _config


def _warn_default_secrets(cfg: StreamPipelineConfig) -> None:
    """기본 시크릿 사용 시 경고 로그"""
    if not os.getenv("MINIO_SECRET_KEY"):
        logger.warning(
            "MINIO_SECRET_KEY not set - using default. "
            "Set the environment variable for production use."
        )
    if not os.getenv("POSTGRES_PASSWORD"):
        logger.warning(
            "POSTGRES_PASSWORD not set - using default. "
            "Set the environment variable for production use."
        )
